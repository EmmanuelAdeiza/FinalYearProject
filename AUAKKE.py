...
def generate_candidate_keywords1(stop_word_file_path):
    phrase_list = []
    ...
    def acceptable_word(word):
        ...
        accepted = bool(2 <= len(word) <= 40
                        and word.lower() not in stop_word_list)
        return accepted
    for subtree in tree.subtrees(filter = lambda t: t.label()=='NP'):
        for leaf in subtree.leaves():
            term = [normalise(w) for w in leaf if acceptable_word(w)]
            for term_item in term:
                print(term_item)
                for word in term_item:
                    # print(word)
...
def calculate_word_scores_with_pos_tagging(word_frequency):
    # print(word_frequency)
    for item in word_frequency.keys():
        word_frequency_dict = dict(nltk.pos_tag([item]))
        # print(word_frequency_dict)
        # print(word_frequency_dict[item])
        if (word_frequency_dict[item] == 'NN') or (word_frequency_dict[item] == 'NNS') \
                or (word_frequency_dict[item] == 'NNP') or (word_frequency_dict[item] == 'NNPS') :
            # print(word_frequency[item])
            word_frequency[item] += 1
            # print(word_frequency[item]
    # print(word_frequency)

    pos = ['noun', 'pronoun', 'verb', 'adverb', 'adjective', 'preposition', 'interjection', 'gerund', 'conjunction']
...
class Rake(object):
    def __init__(self, stop_words_path, min_char_length=1, max_words_length=4, min_keyword_frequency=1):
...
# print(phraseList)
generate_candidate_keywords1(stoppath)
# # calculate individual word scores
initial_wordscores = calculate_word_scores(phraseList)


wordscores = calculate_word_scores_with_pos_tagging(initial_wordscores)

# generate candidate keyword scores
keywordcandidates = generate_candidate_keyword_scores(phraseList, initial_wordscores)
